...
3. 攻撃の「無事」とHP/MP増減結果は1,000ms、付与された状態効果は1件につき500ms、攻撃以外の結果表示は表示プロファイルの`resolutionHoldMs`に従う。`PROJECT`
...
7. HP/MP増減オーバーレイは対象プレイヤー側の領域（防御カード表示位置）へ出し、`+10hp`、`-5hp`、`+10mp`のような差分だけを表示する。正の差分は緑、負の差分は赤とし、下からフェードインして1,000ms表示する。`PROJECT`
8. 「許す」は負の選択として赤で表示し、従来どおり「許す」（500ms）からダメージ結果（1,000ms）の順で表示する。`PROJECT`
...
3. 守護神の後に病の5%悪化判定、その後に病ダメージまたは天国病の回復を、対象側のHP差分オーバーレイ（赤い`-Nhp`または緑の`+Nhp`、1,000ms）として表示する。`RULE`
